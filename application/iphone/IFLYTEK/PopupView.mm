...
- (void) setText:(NSString *) text andNeedToReturnToKeyBoard:(bool)flag
{
    ...
    frame =  CGRectMake((_parentView.frame.size.width - frame.size.width)/2, -(_textLabel.frame.size.height+20)/2 + _parentView.frame.size.height-g_deviceData.m_screenHeight/2, _textLabel.frame.size.width+10, _textLabel.frame.size.height+20);
    self.frame = frame;
    [UIView animateWithDuration:3.0
                     ...
                     completion:^(BOOL finished){
                         if (_queueCount == 1)
                         {
                             if(_needReturnToKeyboard)
                             {
                                 [self.ParentView changeToKeyboard];
                             }

...
- (id)init
{
    ...
    {
#ifdef __USING_SPEECH_INPUT__
        [self setFrame:CGRectMake(0, 0, g_deviceData.m_screenWidth, 33)];
        ...
        [self addSubview:singleTextView];
...
#else
        [self setFrame:CGRectMake(0, 0, g_deviceData.m_screenWidth, 31)];
...
- (void)showKeyBoard
{
    ...
    //[viewController needScrollV:true];//暂时解决软键盘退出后游戏鼠标事件不响应问题
        ...
        else
        {
            ...
            if(g_deviceData.m_isPad)
            {
                    if(popoverController==NULL)
                    {
                        ...
                        [popoverController presentPopoverFromRect:grt inView:self permittedArrowDirections:UIPopoverArrowDirectionDown animated:YES];
                    }
                    else
                    {

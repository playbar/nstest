...
//int XString8::Compare(XPCTSTR pSrt,XPCTSTR pDst,int nLen,XU8 bLowcase)
//{
//	if(pSrt==XNULL)
//	{
...
//}
//int XString8::Compare(XPCTSTR pStr,XBOOL bLowcase,int nLen)
//{
...
#ifdef _SYMBIAN
			TText* pData;
			pData=ZCConvert::ConvertToUnicode((TText8*)pStr,iLen);
			//iLen=pData->iLen;
#elif defined(_WIN32)
...
int XString16::GBToUCS(const char*buf,XU16*dst,int nCount)
{
	//memset(dst,nCount+1);
#ifdef _SYMBIAN
	unsigned short *temp = ZCConvert::ConvertToUnicode((unsigned char*)buf,nCount);
	int iLen = wcslen((wchar_t*)temp);
	wcscpy((wchar_t*)dst,(wchar_t*)temp);
	dst[iLen]=0x00;
	delete temp;
	return iLen;
#else
...
int XString16::UTF8ToUCS(const char*buf,XU16*dst,int nCount)
	{
	unsigned short *temp = ZCConvert::ConvertFromUTF8((unsigned char*)buf,nCount);
	int iLen = wcslen((wchar_t*)temp);
	wcscpy((wchar_t*)dst,(wchar_t*)temp);
	dst[iLen]=0x00;
	delete temp;
...
#ifdef _SYMBIAN
			TText* pData;
			pData=ZCConvert::ConvertToUnicode((TText8*)pStr);
#elif defined(_WIN32)
...
#ifdef _SYMBIAN
	TText* tempData;
	tempData=ZCConvert::ConvertFromUTF8((TText8*)pData);
	memmove(m_pData,tempData,(wcslen((wchar_t*)tempData)+1)*2);
	delete tempData;
#elif defined(_WIN32)
	MultiByteToWideChar(CP_UTF8,0,pData,nLen,m_pData,nLen);
#endif
	m_nSize=wcslen((wchar_t*)m_pData)+1;
	FreeExtra();
}
...
int XString16::Find(XPCWSTR pData,XUINT nBegin)
{
	...
	if(nBegin<0) nBegin=0;
#ifdef _SYMBIAN
	/*int l=gg_wcslen(pData),i;
	XPCWSTR pSrt=m_pData;
	int len=GetLength()-l;
	while(len>=0)
		{
		 if(Compare(pSrt,pData,l==0)) return pSrt-m_pData;
		 pSrt++;
		 len--;
		}
	return -1;*/
...
#ifdef _SYMBIAN
	//XWCHAR* lpwBuf;
//	XString16 strTmp(m_pData); 
//	XTCHAR* lpDestBuf;
	//lpwBuf=ZCConvert::ConvertFromUTF8((TText8*)m_pData);
//	lpDestBuf=(XTCHAR*)ZCConvert::ConvertToGBK((TText16*)strTmp.GetData());
	XTCHAR* lpDestBuf;
	lpDestBuf = (XTCHAR*)ZCConvert::ConvertUTF8toGBK((TText8*)m_pData);
#elif defined(_WIN32)
	int nLen = m_nSize;
...
int XString16::Find(XWCHAR c,XUINT nBegin)
{
	if(nBegin>=m_nSize-1) return -1;
	if(nBegin<0) nBegin=0;
	XPCWSTR p=(unsigned short*)wcschr((wchar_t*)m_pData+nBegin,(wchar_t)c);
	if(p==XNULL) return -1;
	return p-m_pData;
}	
...
int XString16::Compare(XPCWSTR pSrt,XPCWSTR pStr,XBOOL bLowcase,int nLen)
{
	...
	if(bLowcase)
		...
		for(i=0;i<nLen;i++)
		{
			int v=pSrt[i]-pStr[i];
			if(v) return v;
			else if(!pSrt[i]) return v;
		}
		}
	return 0;*/
	if(nLen>0)
			return bLowcase?wcsnicmp((wchar_t*)pSrt,(wchar_t*)pStr,nLen):gg_wcsncmp(pSrt,pStr,nLen);
	else return bLowcase?wcsicmp((wchar_t*)pSrt,(wchar_t*)pStr):gg_wcscmp(pSrt,pStr);
#elif defined(_WIN32)	
	if(nLen>0)
...
int XString16::Compare(XPCWSTR pStr,XBOOL bLowcase,int nLen)const
{
	...
	return 0;*/
	if(nLen>0)
			return bLowcase?wcsnicmp((wchar_t*)m_pData,(wchar_t*)pStr,nLen):gg_wcsncmp(m_pData,pStr,nLen);
	else return bLowcase?wcsicmp((wchar_t*)m_pData,(wchar_t*)pStr):gg_wcscmp(m_pData,pStr);
#elif defined(_WIN32)
	if(nLen>0)

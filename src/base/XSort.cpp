...
XINT XSort::CompareString8(XPCTSTR strKey,XPCTSTR strValue,XU8 nMode)
{
	if(strKey==strValue) return 0;
	...
	else
	{
		...
		case XSS_NOCASE:rs=XString8::Compare(strKey,strValue,0,XTRUE);					 
			 break;
		default:
		case XSS_NORMAL:rs=XString8::Compare(strKey,strValue,0,XFALSE);
		}
	}
	if(rs>0) rs=1;
	else if(rs<0) rs=-1;

...
//		if(ret==SOCKET_ERROR){closesocket(m_hSocket);m_hSocket=XNULL;return XC_ERROR;}
//Modify Sync
...
{
	if(m_hSocket==XNULL) return NOTCONNECTED;
	...
	FD_SET(m_hSocket,&readfds); // indicate that we want to
...
//	int nv=select(1, &readfds, NULL, NULL, &Timeout);
//	if(nv>0) 
//		return REVOK;
//	else if(nv==SOCKET_ERROR) return REVERROR;
...
//	int n=sizeof(addr);
	...
	return recvfrom(m_hSocket,(char*)pData,nSize,0,&addr,&n);
}

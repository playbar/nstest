...
//int XRingBlock::countSet = 0;
//int XRingBlock::countGet = 0;
...
{
	//return m_pDevice->nChannels;
    return 1;
}
int XMicrophone::GetBitsPerSample()
{
    return 16;
	//return m_pDevice->wBitsPerSample;
}
...
bool XMicrophone::SetLoopback(bool bLoop,XSoundMix*pMix,bool bClose)
{
	...
	if(lock.Lock(XSHORTTIME))
	{
		...
		{		
			...
			MicrophoneRecordState*pFmt=m_pDevice->GetFormat();
			m_sound.format = sndCompressNone;
			if(pFmt->dataFormat.mChannelsPerFrame==2)
				m_sound.format |= sndStereo;
			if(pFmt->dataFormat.mBitsPerChannel==16)
				m_sound.format|=snd16Bit;
			switch((int)pFmt->dataFormat.mSampleRate)
			{
				default:
				case XPCM_8K:
				case XPCM_11K:
			...
			XSoundChannel*channel = m_pChannel;
            if (channel == XNULL)
            {
...
bool XMicrophone::DataIn(void *bufRef, void *desc)
{
    AudioQueueBufferRef bufferRef = (AudioQueueBufferRef)bufRef;
    MicrophoneRecordState* descr = (MicrophoneRecordState *)desc;
	//WAVEHDR*pHDR=(WAVEHDR*)pData;
	if(bufferRef->mAudioDataByteSize<=0) return false;
	//WAVEFORMATEX*pFmt=m_pDevice->GetFormat();
	if(m_bLoopback)
	{
		XLock lock(&m_lock);
		if(lock.Lock(XSHORTTIME))
		{
			int iSamples=bufferRef->mAudioDataByteSize;
			if(descr->dataFormat.mChannelsPerFrame==2) iSamples>>=1;
			if(descr->dataFormat.mBitsPerChannel==16) iSamples>>=1;
			m_blocks.SetBlock(bufferRef->mAudioData,bufferRef->mAudioDataByteSize,iSamples);
	...
	int gain=0;
	if(descr->dataFormat.mBitsPerChannel==8)
	{
		XS8* pData=(XS8*)bufferRef->mAudioData;
		int i,iCount=bufferRef->mAudioDataByteSize;
		for(i=0;i<iCount;i++)
		{
			XS16 c=pData[i]+128;
            //printf("%X\n",pData[i]);
	...
	{
		XS16* pData=(XS16*)bufferRef->mAudioData;
		int i,iCount=bufferRef->mAudioDataByteSize/2;
		for(i=0;i<iCount;i++)
		{
	...
	else if(m_bActive)
	{
		...
		else if(t-m_nLastTime>=this->m_silienceTimeout)
		{
			m_bActive=false;
			this->m_activeLevel=gain;
			this->Call(MICROPHONE_DEACTIVE);
		}
		this->m_activeLevel=gain;
	}
	
	Call(MICROPHONE_DATA,bufferRef->mAudioData,bufferRef->mAudioDataByteSize);
	return true;
}

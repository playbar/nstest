...
//#ifdef __APPLE__
//void MixCallback(HWAVEOUT hwo,	UINT uMsg, XU32 dwInstance, XU32 dwParam1, XU32 dwParam2)
//#else
...
bool XSoundMix::hasActiveQueueBuffer()
{
	for (size_t i = 0; i < m_totalAllocedQueueBuffer.size(); ++i)
	{
		if (m_totalAllocedQueueBuffer[i].m_index != -1)
			return true;
	}
	return false;
}
...
void XSoundMix::AQOutputCallback(void* userData, AudioQueueRef param1, AudioQueueBufferRef param2)
{
	XSoundMix* soundMix = (XSoundMix*)userData;
	
	XLock lock(&soundMix->m_playLock);
	if(!lock.Lock(XSHORTTIME))
		return;

	int findIndex = -1;
	for (size_t i = 0; i < soundMix->m_totalAllocedQueueBuffer.size(); ++i)
	{
		if (soundMix->m_totalAllocedQueueBuffer[i].m_queueBuffer == param2)
		{
			findIndex = soundMix->m_totalAllocedQueueBuffer[i].m_index;
			soundMix->m_totalAllocedQueueBuffer[i].m_index = -1;
		}
	}
	assert(findIndex != -1);
	...
	if (!soundMix->IsDeviceOpen())
	{
		lock.UnLock();
		return;
	}
	
	soundMix->playEvent.SetEvent();
	
	if (soundMix->bPause)
	{
//		if (!soundMix->hasActiveQueueBuffer())
//			soundMix->CloseNativeDevice();
		lock.UnLock();
		return;
	}
...
//	int nSent = 0;
//	for ( int i = 0; i < soundMix->nBuffers; i++ ) {
//		_XWAVEHDR* hdr = (_XWAVEHDR*)(soundMix->waveHDR+i);
//		if ( (hdr->dwFlags & WHDR_DONE) || !(hdr->dwFlags & WHDR_PREPARED) ) {
//			if ( soundMix->buffer[i] )
//			{
//				soundMix->BuildAndWrite( &soundMix->waveHdr[i], false );
//			}
//			// 				SendBuffer(i);
//			nSent++;
//		}
//	}
//	
//	if ( nSent > soundMix->nBuffers-2 ) 
//		soundMix->EnlargeBuffers();
...
XSoundMix::QueueBufferData* XSoundMix::getFreeQueueBuffer()
{
	for (size_t i = 0; i < m_totalAllocedQueueBuffer.size(); ++i)
	{
		if (m_totalAllocedQueueBuffer[i].m_index == -1)
			return &m_totalAllocedQueueBuffer[i];
	}
	if (m_totalAllocedQueueBuffer.size() > 10)
		return 0;
	m_totalAllocedQueueBuffer.push_back(QueueBufferData());
	QueueBufferData* data = &m_totalAllocedQueueBuffer[m_totalAllocedQueueBuffer.size() - 1];
	data->m_index = -1;
	assert(m_queue);
	OSStatus status = AudioQueueAllocateBuffer((AudioQueueRef)m_queue, 4096, (AudioQueueBufferRef*)&data->m_queueBuffer);
	return data;
}
...
//	dataFormat.mSampleRate = format->Rate();
...
//	dataFormat.mReserved = 0;
	...
	dataFormat.mBitsPerChannel = 16;
	dataFormat.mReserved = 0;
	OSStatus status = AudioQueueNewOutput(&dataFormat, AQOutputCallback, this, 0, kCFRunLoopCommonModes, 0, (AudioQueueRef*)&m_queue);
	if (status == 0)
	{
		isOpen = true;
	}
	return status == 0;
//	AudioQueueAllocateBuffer(m_audioData->m_queue, 1000, &m_audioData->m_buffers[i]);
//	AQOutputCallback(this, m_audioData->m_queue, m_audioData->m_buffers[i]);
...
void XSoundMix::CloseNativeDevice()
{
	...
	isOpen = false;
	assert(m_queue);
	AudioQueueStop((AudioQueueRef)m_queue, true);
	for (size_t i = 0; i < m_totalAllocedQueueBuffer.size(); ++i)
	{
		AudioQueueFreeBuffer((AudioQueueRef)m_queue, (AudioQueueBufferRef)m_totalAllocedQueueBuffer[i].m_queueBuffer);
	}
	m_totalAllocedQueueBuffer.clear();
	AudioQueueDispose((AudioQueueRef)m_queue, true);
	m_queue = 0;
	...
	for ( int i = 0; i < nBuffers; i++ ) {
		int limit = 50;
		while ( limit-- ) {
			int res = waveOutUnprepareHeader(hW, (_XWAVEHDR*)(waveHDR+i), sizeof(_XWAVEHDR));
...
void XSoundMix::BuffersReady()
{
	...
	if (IsDeviceOpen())
	{
		AudioQueueStart((AudioQueueRef)m_queue, NULL);
	}
...
#ifdef __APPLE__
	QueueBufferData* data = getFreeQueueBuffer();
	if (!data)
	{
		//printf("too many queue buffer data\n");
		hdr->dwFlags |= WHDR_PREPARED;
		hdr->dwFlags |= WHDR_INQUEUE;
		return;
	}
	data->m_index = i;
	assert(hdr->dwBufferLength <= 4096);
	AudioQueueBufferRef inBuffer = (AudioQueueBufferRef)data->m_queueBuffer;
	memcpy(inBuffer->mAudioData, hdr->lpData, hdr->dwBufferLength);
	inBuffer->mAudioDataByteSize = hdr->dwBufferLength;
	OSStatus status = AudioQueueEnqueueBuffer((AudioQueueRef)m_queue, inBuffer, 0, 0);
	if (status != 0)
	{
		//printf("audioQueueEnqueBuffer wrong , error code = %d\n", status);
	}
	else 
	{
		hdr->dwFlags |= WHDR_PREPARED;
...
//	}
	...
	// add hdr->pcmData 
#ifdef WIN32
	res = waveOutPrepareHeader((HWAVEOUT)hWaveOut, hdr, sizeof(WAVEHDR));
	//FLASHASSERT(res == MMSYSERR_NOERROR);
...
// The time should be relative to the start of the envelope
{
	...
	int ie = 0;
	XS32 dLevel=0,level=sndMaxLevel*v;//(sndMaxLevel*v/100);
	while ( n > 0 ) {
		// Find the proper segment of the envelope
		// Set up a new envelope segment
		// Note: The level difference equation is 2.30 fixed point numbers
...
XBOOL XSoundMix::SetVolume(int v)
{
	...
	if (m_queue)
	{
		AudioQueueSetParameter((AudioQueueRef)m_queue, kAudioQueueParam_Volume, ((float)v) / 65535);
	}
#endif
	return XTRUE;
}
...
int XSoundMix::GetVolume()
{
	...
	if (m_queue)
	{
		...
		AudioQueueGetParameter((AudioQueueRef)m_queue, kAudioQueueParam_Volume, &value);
		v = value * 65535;
	}
#endif

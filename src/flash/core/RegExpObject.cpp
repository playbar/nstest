...
//#if (__CORE_VERSION__>=0x02077000)
			int utf8Length=utf8Subject.length();
//#endif
			ArrayObject* matchArray;
			while (true)
			{
...
//#endif
				...
				//[Note X]
//#if (__CORE_VERSION__>=0x02076000)
				m_lastIndex = matchIndex+matchLen;//utf8Subject.toIndex(matchIndex+matchLen);
...
//#endif
				...
				//[Note X]
//#if (__CORE_VERSION__>=0x02077000)
				if(m_lastIndex>=utf8Length)
//#else
//				if(m_lastIndex>=subject->length())
//#endif
					break;
			}

...
#ifndef __MethodEnv_inlines_H__  // add by hgl 
#define __MethodEnv_inlines_H__
...
//[Note X]
REALLY_INLINE void MethodEnv::setActivationOrMCTableRef(void *ptr, int32_t type)
{
    AvmAssert((uintptr_t(ptr) & 7) == 0);
    WB(core()->GetGC(), this, &activationOrMCTableRef, (uintptr_t)ptr | type | (activationOrMCTableRef & kIsScriptEnv));
}
...
/*REALLY_INLINE bool MethodEnv::isOverwrite()
{
	return (activationOrMCTable & kIsOverwriteEnv) != 0;
}
REALLY_INLINE void MethodEnv::setOverwrite(bool b)
{
	if(b) activationOrMCTable|=kIsOverwriteEnv;
	else  activationOrMCTable&=~kIsOverwriteEnv;
}*/

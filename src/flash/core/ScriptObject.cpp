...
namespace avmplus
{
		...
		delegate(_delegate)
	{
		...
		//int s1=MMgc::GC::Size(this);
		//int s2=vtable->traits->getTotalSize();
	...
	ScriptObject::~ScriptObject()
	{
		...
		//StUTF8String s(vtable->traits->name());
		//if(strcmp(s.c_str(),"TweenLite")==0)
		//	int v=0;
		vtable->traits->destroyInstance(this);
	}
#ifdef _ANALY
	void ScriptObject::Report(XSortMemName*pInfo)
	{
    ...
    {
		//if(value==181193783)//			if(sp[0]==181193783)
		//		int v=0;
		...
		if (traits()->needsHashtable())
		{
			...
			//}
			//StUTF8String ss(s);
			//if(strcmp(ss.c_str(),"constructor")==0)
			//	int v=0;
			AvmAssert(s->isInterned());
			Atom ival = s->getIntAtom();
			if (ival)
	...
	Atom ScriptObject::getSlotAtom(uint32_t slot,Traits*getTraits)
	{
		Traits* traits = getTraits?getTraits:this->traits();
...
/*#ifdef _WINEMU
				...
				/*if(slot==25)
				{
					...
					if(strcmp(s.c_str(),"BinaryItem")==0)
					{
						if(AvmCore::isNullOrUndefined(value)&&
							!AvmCore::isNullOrUndefined(*((Atom*)p)))
							int v=0;
					}
					//int v=0;
				}*/
		...
		default:
			{
				...
				{
					AvmAssert(sst == SST_scriptobject);//[Note X]
	...
	Stringp ScriptObject::formatToString(Stringp strsp,Stringp streq,Stringp strqt,Stringp strName,ArrayObject*pArgs)//AvmMethodEnv, uint32_t argc, AvmBox* argv)
	{

		Stringp	strResult=strName?strName:(Stringp)core()->kEmptyString; //modify by hgl
		int i,nc=pArgs->getLength();
		AvmCore* core = this->core();
		Toplevel* toplevel = this->toplevel();
		...
		{
			if(i||strName)
				strResult=strResult->concatStrings(strResult,strsp);
			//strName=pArgs->[i];
			Atom atom = pArgs->getUintProperty(i);
			//bool bString=
			if (AvmCore::istype(atom, STRING_TYPE))
			{
				strName=core->atomToString(atom);
				strResult=strResult->concatStrings(strResult,strName);
				strResult=strResult->concatStrings(strResult,streq);
							
				Multiname tempname(core->findPublicNamespace(), strName);
				//Atom atom_out=atom();
				Atom result = toplevel->getproperty(this->atom(),&tempname,vtable);
				bool bString=AvmCore::istype(result,STRING_TYPE);
				if(bString&&strqt)
					strResult=strResult->concatStrings(strResult,strqt);
				strName=core->string(result);
				strResult=strResult->concatStrings(strResult,strName);
				if(bString&&strqt)
					strResult=strResult->concatStrings(strResult,strqt);
			}
	...
	Stringp ScriptObject::formatString(Stringp strp,Stringp stre,Stringp strqt,bool noObject,bool noName,bool justVar)
	{		
		Stringp strResult=(noName||justVar)?(Stringp)core()->kEmptyString:vtable->traits->name();
		...
		for(;;)
		{
			...
			if(str)
			{
				...
				switch (AvmCore::bindingKind(bd))
				{
					...
					case BKIND_GETSET:
						if(!justVar)
						{
							...
							if(noObject&&atomKind(atom)==kObjectType)
								atom=0;
							//atom=toplevel()->op_call(method, 0,&atomv);//argc, atomv);
							//atom=this->getStringProperty(str);
						};
				...
				{
					//if(iSet)
					bool bString=atomKind(atom)==kStringType;
					if(iSet||(!noName&&!justVar))
						strResult=strResult->concatStrings(strResult,strp);
					strResult=strResult->concatStrings(strResult,str);
					strResult=strResult->concatStrings(strResult,stre);
					if(bString&&strqt)
						strResult=strResult->concatStrings(strResult,strqt);
					strResult=strResult->concatStrings(strResult,core()->string(atom));
					if(bString&&strqt)
						strResult=strResult->concatStrings(strResult,strqt);
					iSet++;
					//strResult->append(str);
					//strResult->append(core()->intern(atom));						
				}
			}
			}
			return strResult;

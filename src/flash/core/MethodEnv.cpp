...
namespace avmplus
{
	...
	inline Atom MethodEnv::endCoerce(int32_t argc, uint32_t *ap, MethodSignaturep ms)
	{
		...
		//Stringp name=this->_scope->vtable()->traits->name();
		//Namespacep p=this->_scope->vtable()->traits->ns();
		//StUTF8String ss(name);
		//const char *pTmp = ss.c_str();//char*
		////LOGWHEREMSG( pTmp );
		//if ( p != NULL )
		//{
		//	Stringp domain = NULL;
							...
							Atom* scopes) const
    {
		...
		//StUTF8String s(ctraits->name());
		//if(strstr(s.c_str(),"CrossDomainRSLItem"))
		//	int v=0;
		...
		/*MethodEnv*env=(MethodEnv*)this;
		if(env->getOverMethodEnv())
		{
			ivtable->overMethod=env->getOverMethodEnv();
			//int n=ivtable->methods
		}*/
    ...
    {
		//if(strcmp((char*)multiname->getName()->GetBuffer()->p8,"tmp")==0)
		//	int v=0;
		/*StUTF8String ss(multiname->getName());
		if(strcmp(ss.c_str(),"_chatIP")==0)
			int v=0;
		if(strcmp(ss.c_str(),"_gameIP")==0)
			int v=0;*/
		Toplevel* toplevel = this->toplevel();
								 ...
								 Atom* withBase)
    {
		...
		{
			//Namespacep ns=multiname->getNamespace();
			//Stringp s=ns->getURI();
			//StUTF8String ss(multiname->getName());
			//if(strcmp(ss.c_str(),"Domain")==0)
			//	int v=0;
	...
	Atom MethodEnv::findglobalproperty(Atom target_global, const Multiname* multiname)
	{
		...
		Stringp url=multiname->getNamespace()->getURI();
	...
	WeakKeyHashtable *MethodEnv::getMethodClosureTableRef()
	{
		int type = getType();
		if(!activationOrMCTableRef)
		{
			WeakKeyHashtable *wkh = new (core()->GetGC()) WeakKeyHashtable(core()->GetGC());
			setActivationOrMCTableRef(wkh, kMethodTable);
			return wkh;
		}
		else if(type == kActivation)
		{
			...
			setActivationOrMCTableRef(amtp, kActivationMethodTablePair);
			return wkh;
		}
		else if(type == kActivationMethodTablePair)
		{
		...
		return (WeakKeyHashtable*)(activationOrMCTableRef&~7);
	}

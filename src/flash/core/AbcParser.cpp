...
namespace avmplus
{
									...
									Namespacep protectedNamespace)
	{
		...
		//if(strcmp(ss.c_str(),"ProfilerContent")==0)
		//	int v=0;
		...
		for (uint32_t i=0; i < nameCount; i++)
        {
			...
			//StUTF8String ss(name);
			//if(strstr(ss.c_str(),"CrossDomainRSLItem"))
			//	int v=0;
    ...
    void AbcParser::parseCpool(API api)
    {
		...
		for(uint32_t i = 1; i < string_count; ++i)
		{
			...
			//if(strncmp((char*)pos,"ProfilerContent",10)==0)
			//	int v=0;
			// check to see if we are trying to read past the file end or the beginning.
    ...
    void AbcParser::addNamedTraits(NamespaceSetp nss, Stringp name, Traits* itraits)
    {
        ...
        if (!ns->isPrivate()) {
			...
			//int v=0;
			//[Note X]
			//Domain*pBase=domain->getNamedScriptDomain(name,ns);
			//if(pBase&&pBase->IsOverWrite())
			//	pBase->addNamedTrait(name,ns,itraits);
            // duplicate class
		...
		else 
		{
			...
			else
			{
				...
				if(pBase&&pBase->IsOverWrite())
				{
					...
					//pBase->addNamedScript(name,ns,script);
				//int v=0;
	...
	bool AbcParser::parseInstanceInfos()
	{
		...
		for (uint32_t i=0; i < classCount; i++)
		{
			...
			//StUTF8String ss(name);
			//if(strcmp(ss.c_str(),"AccessibilityImplementation")==0)
			//	int v=0;
...
//				int v=0;
			...
			// XSetHashTable
			//itraits->set_needsHashtable(true);//(flags&1) == 0);
...
//				int v=0;
	...
	{
//#if defined(_SYMBIAN) && defined(__GCCE__)
...
//		double data;
//		#ifndef XU32
//		#define XU32 unsigned long
//		#endif
//		XU32*pDouble=(XU32*)&data;
//		XU32*ps=(XU32*)p;
//		pDouble[0]=ps[0];
//		pDouble[1]=ps[1];

...
namespace avmplus
{
	...
	Domain* Domain::getNamedTraitsDomain(Stringp name,Namespacep ns)
	{
		if(m_base)
		{
			Domain* domain=m_base->getNamedTraitsDomain(name,ns);
			if(domain) return domain;
		}
		if(m_namedTraits->get(name,ns))
			return this;
		return NULL;
	}
	Domain* Domain::getNamedScriptDomain(Stringp name,Namespacep ns)
	{
		if(m_base)
		{
			Domain* domain=m_base->getNamedScriptDomain(name,ns);
			if(domain) return domain;
		}
		if(m_namedScripts->get(name,ns))
			return this;
		return NULL;
	}

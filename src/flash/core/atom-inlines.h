...
#ifdef _XBIGINTERGER_
	REALLY_INLINE bool atomIsIntptrAll(Atom atom)
	{
		return (atomKind(atom) == kIntptrType)|(atomKind(atom)==kBigIntptrType);
	}
	REALLY_INLINE bool atomIsBothIntptrAll(Atom a,Atom b)
	{
		return atomIsIntptrAll(a)&&atomIsIntptrAll(b);
	}
#endif
...
#elif _WINEMU64
		_int64 const i = atomGetIntptr(atom);
        uint32_t const u32 = uint32_t(i);
        return i == intptr_t(u32);
#else
...
#ifdef _WINEMU64
	REALLY_INLINE _int64 atomGetIntptr(Atom a)
    {
        //AvmAssert(atomIsIntptr(a));
        //AvmAssert(atomIsValidIntptrValue((a) >> 3));
        return a >> 3;
    }
#else
...
#ifdef _WINEMU64
    REALLY_INLINE bool atomIsValidIntptrValue(const int64 i)
    {
        return (((i << 8) >> 8) == i);
    }

    REALLY_INLINE bool atomIsValidIntptrValue_u(const unsigned _int64 u)
    {
        const unsigned _int64 MASK = ~((atomMaxIntValue)>>1);
        return (u & MASK) == 0;
    }	
#else

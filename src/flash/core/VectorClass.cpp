...
namespace avmplus
{
	...
	{
		//if(deleteCount+insertPoint>m_length)
		//	deleteCount=m_length-insertPoint;
		...
		if (insertCount)
		{
			...
			{
				if(offset+insertCount > vec_args->m_length)
					insertCount = vec_args->m_length-offset;
			...
			{
				//if(insertCount+offset>=so_args->m_length)
				//	insertCount=so_args->m_length-offset;
				for (uint32 i=0; i<insertCount; i++)
				{

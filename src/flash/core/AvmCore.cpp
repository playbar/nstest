...
//extern void _FilterValue(const void*,const void*,const void*);
namespace avmplus
{
	...
	void AvmCore::exportDefs(Traits* scriptTraits, ScriptEnv* scriptEnv)
	{
		...
		while (iter.next())
		{
			...
			//if(strstr(ss.c_str(),"load"))
			//	int v=0;
			/*{//[Note X]
				DomainEnv*base=domainEnv->base();
				while(base)
				{
					if(base->isOverwrite())
					{
						ScriptEnv*script=base->getNamedScript(name,ns);
						if(script)
							script->setOverMethodEnv(scriptEnv);
							//script->setOverwrite(true);
					}
					base=base->base();
				}
			}*/
...
22. Return false.
	*/
	...
	void AvmCore::throwException(Exception *exception)
	{
		...
		// exception.
		/*avmplus::StackTrace*trace=exception->getStackTrace();
		Stringp s=trace->format(this);
		StUTF8String ss(s);
		if(strstr(ss.c_str(),"updateParent")!=0)
			int v=0;*/
		//int v=0;
	...
	String* AvmCore::formatErrorMessageV( int errorID, Stringp arg1, Stringp arg2, Stringp arg3)
	{
		...
		if(arg1==NULL)
			arg1=this->kEmptyString;
		if(arg2==NULL)
			arg2=this->kEmptyString;
		if(arg3==NULL)
			arg3=this->kEmptyString;
	...
	void AvmCore::throwErrorV(ClassClosure *type, int errorID, Stringp arg1, Stringp arg2, Stringp arg3)
	{
		...
		//console << out << "\n";
		//StUTF8String ss(out);
		//if(strstr(ss.c_str(),"well-formed")!=0)
		//{
		//	int v=0;
		//}
		//	int v=0;
	...
	*/
    ...
    /*static*/ bool AvmCore::istype(Atom atom, Traits* itraits)
    {
		...
		{
			//for debug
			//StUTF8String s1(itraits->name());
			//if(strstr(s1.c_str(),"IRSprite")!=XNULL)
			//		return false;
...
//[88]   	Digit	   ::=   	
	...
	void AvmCore::ReleaseIntern(uint8_t*buffer,int len)
	{
		//MMgc::GC*gc=this->gc();
		uint8_t*end=buffer+len;
		for(int i=0;i<numStrings;i++)
		{
			if(strings[i]<=AVMPLUS_STRING_DELETED) continue;
			//if(strings[i]->getWidth() != String::k8) continue;
			strings[i]->ResetBuffer(gc,buffer,end);	
			//Buffer* buf=strings[i]->GetBuffer();
			//if(buf->p8>=buffer&&
			//   buf->p8
		}
	}
...
#define MAKE_INT_ATOM(intval) ((intptr_t(intval)<<3) | kIntptrType)
	...
	/*static*/ int32_t AvmCore::integer(Atom atom)
	{
        ...
        if (kind == kIntptrType)
        {
			//if(atom==54)
			//	int vv=0;
			//int v=atomGetIntptr(atom);
			//double fv=integer_d(number(atom));
			//if(v!=fv)
			//	double mv=integer_d(number(atom));
				//int vvv=0;
...
#error("this routine does not work in PowerPC processors");
	...
	{ 
		//[Note X]
		//_FilterValue(container,address,(void*)(atomNew&~7));
	...
	MethodEnv* AvmCore::methodEnv() const
	{
		for (MethodFrame* f = currentMethodFrame; f != NULL; f = f->next)
		{

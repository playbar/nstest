...
#ifdef _ANALY
	void ArrayObject::Report(XSortMemName*pInfo)
	{
		StUTF8String str(vtable->traits->name());
		int nSize = vtable->traits->getTotalSize();
		/*if(m_pDib&&m_pDib->HasBits())
			nSize+=m_pDib->m_nDataLength;
		else if(m_pChar)
		{
			SCharacter*ch=(SCharacter*)m_pChar;
			if(ch->bits.HasBits())
				nSize+=ch->bits.rowBytes*ch->bits.height;
		}*/
		if(m_denseArr.Atoms())
			nSize += m_denseArr.capacity()*sizeof(Atom);
	...
	uint32 ArrayObject::AS3_push(Atom* argv, int argc)
	{
		/*if(argc==1)
		{
			...
			if(atomKind(atom)==kObjectType)
			{
				...
				if(strcmp(s.c_str(),"ResInfo")==0)
				{
					atom=pObj->getStringProperty(core()->internString(core()->newStringLatin1("url")));
					Stringp s=(Stringp)(atom&~7);
					int v=0;
				}
			}
		}*/

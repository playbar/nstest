...
#define DEFER_STORES(...)
    ...
    bool CodegenLIR::prologue(FrameState* state)
    {
		...
		{//for time out break
			LIns *td = loadIns(LIR_ldp,offsetof(AvmCore,m_bBreakOutTime),coreAddr);
			LIns *tc = binaryIns(LIR_pult,InsConst(true),td);
			//LIns *td = InsConstPtr
			//	loadIns(offsetof(AvmCore,m_bBreakOutTime));
			LIns *tb = branchIns(LIR_jf,td);
			callIns(FUNCTIONID(handleInterruptMethodEnv), 1, env_param);

			LIns *begin_label = label();
			verbose_only( if (lirbuf->names) { lirbuf->names->addName(begin_label, "intbegin");  })
			tb->setTarget(begin_label);
		}
...
#endif
				//callIns(FUNCTIONID(interruptCheck),0);

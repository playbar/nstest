 ...
 *
 * ***** END LICENSE BLOCK ***** */
#ifndef __Toplevel_inlines_H__  // add by hgl 2011-05-27
#define __Toplevel_inlines_H__
...
{
	//Stringp url=vtable->traits->ns()->getURI();
    AssertNotNull(base);
...
REALLY_INLINE ClassClosure* Toplevel::getBuiltinClass(int class_id,bool bCreate) const
{
    return _builtinClasses[class_id]
        ? _builtinClasses[class_id]
		: (bCreate?const_cast<Toplevel*>(this)->resolveBuiltinClass(class_id):NULL);
}

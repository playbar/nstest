...
static XU16 m_nSinV[]={0,101,201,302,402,503,603,704,
...
void CalcRotate(XFDOUBLE r, int &sinv, int &cosv,XU8 b360)
{
	int rotate=(int)(r*0x1ff/double_PI);
	if(b360)
	{
		while(rotate<0) rotate+=360;
		rotate=((rotate<<8)+45)/90;
	}
	else
	{
		while(rotate<0) rotate+=0x100;
	}

...
//extern void _FilterValue(const void*containter,const void*address,const void*value);
...
//#ifdef __APPLE__
//		nogc(false),	// modified by handong
...
#  pragma warning(pop)
	...
	{
		GCAssert(item.ptr != NULL);
		//if((uint32_t)item.GetSize()>0xfffff)//.ptr<0x3000000)
		//	int v=0;
...
bail:
...
#ifdef _ANALY
	void GC::Report(XSortMemName*pInfo)
	{
		for(int i=0; i < kNumSizeClasses; i++) {
			containsPointersRCAllocs[i]->Report(pInfo);
			containsPointersAllocs[i]->Report(pInfo);
			noPointersAllocs[i]->Report(pInfo);
		}
		largeAlloc->Report(pInfo);
	}
#endif
	void GC::Finalize()
	{
	...
	void GC::MarkQueueAndStack(bool scanStack)
	{
		if(scanStack) {
			...
			//int _size = (uint32_t)(GetStackTop() - (uintptr_t)_stack);
			//if(_size<0)
			//	int v=0;

			MMGC_GET_STACK_EXTENTS(this, item.ptr, item._size);
			GCAssert(item.ptr != NULL);
			GCAssert((int)item._size>0);
...
#undef ALLOCA_AND_FILL_WITH_SPACES
	...
	static XSortAddr _sortAddr;
	//static XSeekItem*pRoot=NULL;
	void GC::SeekItem(XSeekItem*pItem)
	{
		//m_pSeekItem = pItem;		
	...
	void GC::SeekItem(GCWorkItem &wi,XSeekItem*pItem)
	{
		size_t size = wi.GetSize();
		///if(size==400)
		//	int v=0;
		uintptr_t *p = (uintptr_t*) wi.ptr;
		if(!_sortAddr.Add(p)) return;
...
//					int v=0;
				...
				if((bits2 & ((GCAlloc::kMark|GCAlloc::kQueued)<<shift)) == 0)
				{
					...
					if(block->alloc->ContainsPointers())
					{
						...
						/*if(realItem==_DebugWindow)
						{	
							RCObject*obj=(RCObject*)wi.ptr;
							int v=0;
						}*/
			...
			else if (bits == GC::kGCLargeAllocPageFirst)
			{
				...
				if((b->flags & (GCLargeAlloc::kQueuedFlag|GCLargeAlloc::kMarkFlag)) == 0)
				{
					...
					if((b->flags & GCLargeAlloc::kContainsPointers) != 0)
					{
						...
						//pItem->Push(name);
						SeekItem(GCWorkItem(GetUserPointer(item), itemSize, true),pItem);
						//pItem->Pop(name);
					}
					else
					{
...
//					int v=0;
	...
	{
		//_FilterValue(container,address,value);
	...
	{
		//_FilterValue(NULL,address,value);
	...
	{
		if(gc) {
			gc->SetStackEnter(NULL,doCollect);

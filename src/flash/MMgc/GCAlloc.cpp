...
#ifdef _ANALY
	void GCAlloc::Report(XSortMemName*pInfo)
	{
		//return;
		GCBlock *next = NULL;
		for (GCBlock* b = m_firstBlock; b != NULL; b = next)
		{
			...
			for(uint32_t i=0; i < numInts; i++) 
			{
				...
				for(uint32_t j=0; j<subCount;j++,marks>>=4)
				{
					...
					//if(GetUserPointer(item)==_DebugWindow)
					//	int v=0;
...
//					int v=0;
				...
				uint32_t subCount = i==(numInts-1) ? ((count-1)&7)+1 : 8;
//				if(i==5&&numInts==6&&subCount==7)
//					int v=0;
				for(uint32_t j=0; j<subCount;j++,marks>>=4)
				{
//					if(i==5&&j==6&&numInts==6&&subCount==7)
//						int v=0;
					/*{
						void* item = (char*)b->items + m_itemSize*((i*8)+j);
						if(GetUserPointer(item)==_DebugWindow)
...
//			if(block->GetBits()[GetIndex(block,item)>>3]==72701269)
//				int v=0;

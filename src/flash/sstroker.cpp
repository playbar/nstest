...
void MakeVertLine(CURVE* c, SCOORD x)
{
	...
	if ( c->anchor1.y < -32000 )
		c->anchor1.y = -32000;
	if ( c->anchor2.y >  32000 ) 
		c->anchor2.y = 32000;
...
void SStroker::BeginStroke(S32 thickness, RColor* c)
{
	...
	// Set up the context
	lineThickness = Max(antialias ? nHalf : 1, thickness);
 	isThick = lineThickness > nStep-1;
	color = c;
...
void SStroker::AddStrokeCurve(CURVE* c)
{
	...
	if ( !isThick ) {
		...
		if ( antialias && c->isLine ) {
			CURVE c2 = *c;
			if ( lineThickness == nStep || lineThickness == nStep12 ) {
				if ( c2.anchor1.x == c2.anchor2.x && Abs(c2.anchor1.y - c2.anchor2.y) > nStep12 ) {
					...
					c2.anchor1.x = c2.anchor2.x = (c2.anchor1.x & nMask) + nHalf;

				} else if ( c2.anchor1.y == c2.anchor2.y && Abs(c2.anchor1.x - c2.anchor2.x) > nStep12 ) {
					// Snap a horizontal line to the pixel grid
					c2.anchor1.y = c2.anchor2.y = (c2.anchor1.y & nMask) + nHalf;
				}
			} else if ( lineThickness == nStep8 ) {
				if ( c2.anchor1.x == c2.anchor2.x && Abs(c2.anchor1.y - c2.anchor2.y) > nStep12 ) {
					// Snap a vertical line to the pixel grid, note that we don't need to close the gaps since the
					//	lines to close would be horizontal and have no effect
					c2.anchor1.x = c2.anchor2.x = (c2.anchor1.x + nHalf) & nMask;

				} else if ( c2.anchor1.y == c2.anchor2.y && Abs(c2.anchor1.x - c2.anchor2.x) > nStep12 ) {
					// Snap a horizontal line to the pixel grid
					c2.anchor1.y = c2.anchor2.y = (c2.anchor1.y + nHalf) & nMask;
				}
			}
			StrokeThickCurve(&c2);
		} else {

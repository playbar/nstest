...
{
	//LOGWHERE();
	sem_init((sem_t*)&lock->lock,0, 1 );
//	pthread_mutexattr_t mattr;
//	pthread_mutexattr_init(&mattr);
//	pthread_mutexattr_settype(&mattr, PTHREAD_MUTEX_INITIALIZER);
//	pthread_mutex_init((pthread_mutex_t*)&lock->lock, &mattr );
}

REALLY_INLINE void VMPI_lockDestroy(vmpi_spin_lock_t *lock)
{
	//LOGWHERE();
	sem_destroy( (sem_t*)&lock->lock );
...
{
	//LOGWHERE();
	return sem_trywait((sem_t*)&lock->lock) == 0;
...
{
	//LOGWHERE();
	return sem_post((sem_t*)&lock->lock)== 0;

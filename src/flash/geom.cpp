...
{
	int c,s;
	CalcRotate(r,s,c,XFALSE);
	//double c=cos(r);
	//double s=sin(r);
	m->a=c;//DoubleToFixed(c);
	m->b=s;//DoubleToFixed(s);
	m->c=-m->b;
	m->d=m->a;
	m->tx=m->ty=0;
}
...
{
	if(m->a<fixed_1-MIND||m->a>fixed_1+MIND) return false;
	if(m->d<fixed_1-MIND||m->d>fixed_1+MIND) return false;
	if(m->b>MIND||m->b<-MIND) return false;
	if(m->c>MIND||m->c<-MIND) return false;
	return true;
}

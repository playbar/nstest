...
#ifdef GZIPBUF
local int  get_method   OF((GZBUF in));
local void do_list      OF((GZBUF ifd, int method));
#else
...
#ifdef GZIPBUF
int (*work) OF((GZBUF infile, GZBUF outfile)) = zip; /* function to call */
#else
...
local void do_list(int ifd, int method)
{
    ...
    if (method == DEFLATED && !last_member) {
         ...
         */
#ifdef GZIPBUF
        bytes_in = ifd->Seek((off_t)(-8), SEEK_END);
#else
        ...
        if (bytes_in != -1L) {
            ...
            bytes_in += 8L;
#ifdef GZIPBUF
			if (ifd->Read((char*)buf, sizeof(buf)) != sizeof(buf)) {
#else
...
#ifdef GZIPBUF
// based on the original treat_file() call
//
int gzip_compress(GZBUF bin, GZBUF bout)
{
	ifile_size = bin->Length();
    clear_bufs(); /* clear input and output buffers */
    part_nb = 0;
	ifd = bin;
	ofd = bout;
...
int gzip_decompress(GZBUF bin, GZBUF bout)
{
	ifile_size = bin->Length();
    clear_bufs(); /* clear input and output buffers */
    part_nb = 0;
...
int gzip_compress_buffer(char * inbuf, int inlen, char * outbuf, int outlen)
{
	CFileAlikeBuffer bin(inlen, inbuf, inlen);
	CFileAlikeBuffer bout(outlen, outbuf);

	return gzip_compress(&bin, &bout);
}

int gzip_decompress_buffer(char * inbuf, int inlen, char * outbuf, int outlen)
{
	CFileAlikeBuffer bin(inlen, inbuf, inlen);
	CFileAlikeBuffer bout(outlen, outbuf);

	return gzip_decompress(&bin, &bout);
}

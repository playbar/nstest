...
int (*write_mem)(int fd, void *buf, unsigned size);
//Transplant end
#endif
...
#endif
{
	...
	bytes_out += insize;
#ifdef GZIPBUF
	insize = in->Read((char *)inbuf, INBUFSIZ);
#else
...
int fill_inbuf(int eof_ok)
{
    ...
    do {
#ifdef GZIPBUF
	len = ifd->Read((char *)inbuf+insize, INBUFSIZ-insize);
#else
	len = read_mem(ifd, (char*)inbuf+insize, INBUFSIZ-insize);
#endif
...
#ifdef GZIPBUF
void write_buf(GZBUF fd, char *buf, unsigned cnt)
#else
void write_buf(int fd, char *buf, unsigned cnt)
#endif
{
    unsigned  n;

#ifdef GZIPBUF
	while ((n = fd->Write(buf, cnt)) != cnt) {
#else
    while ((n = write_mem(fd, buf, cnt)) != cnt) {
#endif // GZIPBUF
#ifdef GZIPBUF
	if (n == (unsigned)(-1) || n == 0 /* insufficient buffer*/) {
#else
	if (n == (unsigned)(-1)) {
#endif
	    write_error();
...
//Transplant start
// int write_mem(int fd, const void *buf, unsigned size)
// {

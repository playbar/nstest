...
//XStream* app_open_file(const char* filename, const char* mode)
//{
//	return fopen(filename, mode);
//}
//
//void app_close_file(XStream* file)
//{
//	fclose(file);
//}
...
static unsigned char read_byte(XStream *file)
{
	XU8 ch;
	bool ret = file->ReadByte(ch);
	if (!ret)
		return 0;
	else
		return ch;
...
//static int write_byte(XStream *file, int ch)
//{
//	return putc(ch, file);
//}

static int read_stream(XStream *file, unsigned char buffer[], int length)
{
//	int count = (int) fread(buffer, 1, length, file);
	bool ret = file->ReadData(buffer, length);
	if(!ret)
		return 0;
	return length;
//	int i = count;
...
//static int write_stream(XStream *file, unsigned char buffer[], int length)
//{
//	return (int) fwrite(buffer, 1, length, file);
//}

int read_gif_int(XStream *file)
{
	int output;
	unsigned char buf[2];
	file->ReadData(buf, 2);
...
//void write_gif_int(XStream *file, int output)
//{
 ...
 */
GifData * read_gif_data(XStream *file)
{
	GifData *data;
	int size;
 ...
 */
//void write_gif_data(XStream *file, GifData *data)
//{
//	if (data) {
...
#ifdef GIF_DEBUG
void print_gif_data(XStream *file, GifData *data)
{
	int i, ch, prev;
 ...
 */
static unsigned char read_gif_byte(XStream *file, GifDecoder *decoder)
{
 ...
 */
static void finish_gif_picture(XStream *file, GifDecoder *decoder)
{
	unsigned char *buf = decoder->buf;
 ...
 */
//static void write_gif_byte(XStream *file, GifEncoder *encoder, int ch)
//{
...
void read_gif_palette(XStream *file, GifPalette *cmap)
{
	int i;
	unsigned char r, g, b;

	cmap->colours = (Colour*)malloc(cmap->length * sizeof(Colour));

	for (i=0; i<cmap->length; i++) {
		r = read_byte(file);
		g = read_byte(file);
		b = read_byte(file);
//		cmap->colours[i].color = rgb(r,g,b);
#ifdef __ANDROID__
#ifdef	_ARGB_PIXEL_
		cmap->colours[i].red = r;
		cmap->colours[i].green = g;
		cmap->colours[i].blue = b;
#else
		cmap->colours[i].red = b;
		cmap->colours[i].green = g;
		cmap->colours[i].blue = r;
#endif
#else
		cmap->colours[i].red = r;
		cmap->colours[i].green = g;
...
//
//void write_gif_palette(XStream *file, GifPalette *cmap)
//{
//	int i;
//	Colour c;
//
...
#ifdef GIF_DEBUG
void print_gif_palette(XStream *file, GifPalette *cmap)
{
	int i;
...
void read_gif_screen(XStream *file, GifScreen *screen)
{
	unsigned char info;
...
//void write_gif_screen(XStream *file, GifScreen *screen)
//{
//	unsigned char info;
//
...
#ifdef GIF_DEBUG
void print_gif_screen(XStream *file, GifScreen *screen)
{
...
void read_gif_extension(XStream *file, GifExtension *ext)
{
	GifData *data;
	int i;
	...
	while (data) {
		...
		i = ++ext->data_count;
		ext->data = (GifData**)realloc(ext->data, i * sizeof(GifData *));
		ext->data[i-1] = data;
		data = read_gif_data(file);
	}
}

//void write_gif_extension(XStream *file, GifExtension *ext)
//{
//	int i;
//
...
#ifdef GIF_DEBUG
void print_gif_extension(XStream *file, GifExtension *ext)
{
	int i;
...
void init_gif_decoder(XStream *file, GifDecoder *decoder)
{
	int i, depth;
	int lzw_min;
	unsigned int *prefix;
 ...
 */
int read_gif_code(XStream *file, GifDecoder *decoder)
{
	int code;
 ...
 */
void read_gif_line(XStream *file, GifDecoder *decoder,
 ...
 */
//void write_gif_code(XStream *file, GifEncoder *encoder, int code)
//{
 ...
 */
//void init_gif_encoder(XStream *file, GifEncoder *encoder, int depth)
//{
 ...
 */
//void write_gif_line(XStream *file, GifEncoder *encoder, unsigned char *line, int length)
//{
...
//void flush_gif_encoder(XStream *file, GifEncoder *encoder)
//{
...
static void read_gif_picture_data(XStream *file, GifPicture *pic)
{
	GifDecoder *decoder;
	long w, h;
	...
	w = pic->width;
	h = pic->height;
	pic->data = (unsigned char**)malloc(h * sizeof(unsigned char *));
	if (pic->data == NULL)
		return;
	for (row=0; row < h; row++)
...
void read_gif_picture(XStream *file, GifPicture *pic)
{
	unsigned char info;
...
//static void write_gif_picture_data(XStream *file, GifPicture *pic)
//{
//	GifEncoder *encoder;
//	long w, h;
//	int interlace_start[] = {0, 4, 2, 1};
...
//void write_gif_picture(XStream *file, GifPicture *pic)
//{
//	unsigned char info;
//
...
#ifdef GIF_DEBUG
static void print_gif_picture_data(XStream *file, GifPicture *pic)
{
	int pixval, row, col;
...
static void print_gif_picture_header(XStream *file, GifPicture *pic)
{
...
void print_gif_picture(XStream *file, GifPicture *pic)
{
...
void read_gif_block(XStream *file, GifBlock *block)
{
...
//void write_gif_block(XStream *file, GifBlock *block)
//{
...
#ifdef GIF_DEBUG
void print_gif_block(XStream *file, GifBlock *block)
{
...
//	// in samsung mobile, release m_data will cause crash
//#else
//	gif->m_data.~vector<Gif::PackedData>();
//#endif

}

void read_gif(XStream *file, Gif *gif)
{
	int i;
	GifBlock *block;
	...
	while (1) {
		...
		if (block->intro == 0x3B) {	/* terminator */
			...
			/* Append the block: */
			i = ++gif->block_count;
			gif->blocks = (GifBlock**)realloc(gif->blocks, i * sizeof(GifBlock *));
...
void read_one_gif_picture(XStream *file, Gif *gif)
{
	int i;
	GifBlock *block;
...
//void write_gif(XStream *file, Gif *gif)
//{
//	int i;
//
...
#ifdef GIF_DEBUG
void print_gif(XStream *file, Gif *gif)
{
	int i;
...
Gif * read_gif_file(XStream *file)
{
	Gif *gif;
//	XStream *file;
...
void Gif::calculateHelperData()
{
	...
	{
		GifBlock* block = blocks[i];
		if (block->intro == 33)
		{
			// this is a ext.
			assert(block->ext);
			GifExtension* ext = block->ext;
			if (ext->marker == 0xf9)
			{
				assert(m_data.size() >= extIndex);
				if (m_data.size() == extIndex)
				{
					m_data.push_back(Gif::PackedData());
				}
				extIndex++;
				assert(m_data.size() == extIndex);

				Gif::PackedData& packedData = m_data[extIndex - 1];
				assert(ext->data_count > 0);
				GifData* data = ext->data[0];
				assert(data->byte_count >= 3);
				packedData.m_disposal = (data->bytes[0] >> 2) & 0x7;
//				int inputFlag = (data->bytes[0] >> 1) & 0x1;
				int delayTime = (data->bytes[2] << 8) + data->bytes[1];
				if (delayTime == 0) delayTime = 20;
				packedData.m_delayTime = delayTime;

				if ((data->bytes[0] & 0x1) != 0)
				{
					int transparent = data->bytes[3];
					packedData.m_transparent = transparent;
				}
			}
		}
		else if (block->intro == 44)
		{
			assert(m_data.size() >= picIndex);
			if (m_data.size() == picIndex)
			{
				m_data.push_back(PackedData());
			}
			picIndex++;
			assert(m_data.size() == picIndex);
			...
			// this is a pic.
			assert(block->pic);
			m_data[picIndex - 1].m_pic = block->pic;
		}
		else
		{
			// dunno what is this.
	...
	assert(picIndex == extIndex || extIndex == 0);
}

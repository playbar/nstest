...
void read_gif_palette(XStream *file, GifPalette *cmap)
{
	...
	cmap->colours = (Colour*)app_alloc(cmap->length * sizeof(Colour));
...
void Gif::calculateHelperData()
{
	...
	for (int i = 0; i < block_count; ++i)
	{
		...
		if (block->intro == 33)
		{
			...
			if (ext->marker == 0xf9)
			{
				assert(m_data.GetSize() >= extIndex);
				if (m_data.GetSize() == extIndex)
				{
					m_data.Add(Gif::PackedData());
				}
				extIndex++;
				assert(m_data.GetSize() == extIndex);
...
//				int inputFlag = (data->bytes[0] >> 1) & 0x1;
		...
		{
			assert(m_data.GetSize() >= picIndex);
			if (m_data.GetSize() == picIndex)
			{
				m_data.Add(PackedData());
			}
			picIndex++;
			assert(m_data.GetSize() == picIndex);
